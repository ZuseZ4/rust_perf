...
fn main() {
    ...
    unsafe {
        kernel_2(array_c, core::f64::consts::PI);
    }
    core::hint::black_box(&array_c);
    unsafe {
        let val: *const c_char = if (*array_c)[0] < 0.1 {
...
#[inline(never)]
unsafe fn kernel_2(x: *mut [f64; 256], k: f64) {
    core::intrinsics::offload(_kernel_2, (x, k))
}

#[cfg(target_os = "linux")]
unsafe extern "C" {
    pub fn _kernel_2(array_b: *mut [f64; 256], k: f64);
}
...
#[inline(never)]
#[rustc_offload_kernel]
pub extern "gpu-kernel" fn _kernel_2(x: *mut [f64; 256], k: f64) {
    unsafe { (*x)[0] = k };
}
